...
byte bar16[][2] = {
    ...
    {0x80,0xff},{0xc0,0x07},{0xe0,0xff},{0xf0,0xff},{0xf8,0x3f},{0xfc,0x7f},{0xfe,0xff},{0xff,0xff}
};

byte bar24[][3] = {
    {0x00,0x00,0x80},{0x00,0x00,0xc0},{0x00,0x00,0xe0},{0x00,0x00,0xf0},{0x00,0x00,0xf8},{0x00,0x00,0xfc},{0x00,0x00,0xfe},{0x00,0x00,0xff},
    {0x00,0x80,0xff},{0x00,0xc0,0xff},{0x00,0xe0,0xff},{0x00,0xf0,0xff},{0x00,0xf8,0xff},{0x00,0xfc,0xff},{0x00,0xfe,0xff},{0x00,0xff,0xff},
    {0x80,0xff,0xff},{0xc0,0xff,0xff},{0xe0,0xff,0xff},{0xf0,0xff,0xff},{0xf8,0xff,0xff},{0xfc,0xff,0xff},{0xfe,0xff,0xff},{0xff,0xff,0xff}
};

byte bar32[][4] = {
    {0x00,0x00,0x00,0x80},{0x00,0x00,0x00,0xc0},{0x00,0x00,0x00,0xe0},{0x00,0x00,0x00,0xf0},{0x00,0x00,0x00,0xf8},{0x00,0x00,0x00,0xfc},{0x00,0x00,0x00,0xfe},{0x00,0x00,0x00,0xff},
    {0x00,0x00,0x80,0xff},{0x00,0x00,0xc0,0xff},{0x00,0x00,0xe0,0xff},{0x00,0x00,0xf0,0xff},{0x00,0x00,0xf8,0xff},{0x00,0x00,0xfc,0xff},{0x00,0x00,0xfe,0xff},{0x00,0x00,0xff,0xff},
    {0x00,0x80,0xff,0xff},{0x00,0xc0,0xff,0xff},{0x00,0xe0,0xff,0xff},{0x00,0xf0,0xff,0xff},{0x00,0xf8,0xff,0xff},{0x00,0xfc,0xff,0xff},{0x00,0xfe,0xff,0xff},{0x00,0xff,0xff,0xff},
    {0x80,0xff,0xff,0xff},{0xc0,0xff,0xff,0xff},{0xe0,0xff,0xff,0xff},{0xf0,0xff,0xff,0xff},{0xf8,0xff,0xff,0xff},{0xfc,0xff,0xff,0xff},{0xfe,0xff,0xff,0xff},{0xff,0xff,0xff,0xff}
};
...
void LCD_SSD1306::setByteSize (BYTE_SIZE size){
    _byte_size = size;
}

void LCD_SSD1306::setDomain(int min, int max)
{
    _min = min;
    _max = max;
}

void LCD_SSD1306::drawDynamic(byte* buffer, byte width, byte height)
{
    ssd1306_command(SSD1306_SETLOWCOLUMN | 0x0);  // low col = 0
    ssd1306_command(SSD1306_SETHIGHCOLUMN | 0x0);  // hi col = 0
    ssd1306_command(SSD1306_SETSTARTLINE | 0x0); // line #0
...
void LCD_SSD1306::drawGraph(int *data, DATA_LENGTH data_length)
{
    byte buffer[data_length * _byte_size];
    for (int i=0;i< data_length;i++){
        ...
        for (int j= 0; j< _byte_size;j++){
            buffer[j * data_length + i] = *(bar + ((shift -1) * _byte_size) + j);
        }
    }
    drawDynamic(buffer, data_length, 8 * _byte_size);
}
